    ...
    if (!fside) {
      ...
      if (name === docFilterNames[2]) {
        filteredData = docDetail.filter(
          (doc) => parseFloat(doc.satisfiedPatients) >= 0.95
        );
    ...
    <div className={classes.Doctor}>
      <div className={classes.Header}>
        {params.leftItem == "Speciality" || params.leftItem == "Online" ? (
          <h1>
            Best {params.rightItem}s in {params.cityName}{" "}
          </h1>
        ) : (
          <h1>
            Best Doctors for {params.rightItem} in {params.cityName}{" "}
          </h1>
        )}
